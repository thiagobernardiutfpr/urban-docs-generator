...
import { ArrowLeft, Download, Eye, FilePenLine, FileText, Loader2, MapPinned, RefreshCw, Save } from "lucide-react";
...
export default function RequestDetail({ requestId, startEditing = false }: { requestId: number; startEditing?: boolean }) {
  ...
  const canContinueWorkflow = ["collecting", "cross_referenced", "ready_for_review", "failed"].includes(request.status);
  ...
  return <div className="mx-auto max-w-[1180px] space-y-6"><div className="flex flex-col gap-4 border-b border-[#dfe4da] pb-5 sm:flex-row sm:items-end sm:justify-between"><div><button onClick={() => setLocation("/solicitacoes")} className="flex items-center gap-1 text-[10px] font-bold text-[#557437] hover:text-[#264f43]"><ArrowLeft className="h-3.5 w-3.5" />Voltar para solicitações</button><p className="mt-4 font-mono-ui text-[10px] uppercase tracking-[.17em] text-[#627d5b]">Processo {request.protocol}</p><h1 className="mt-2 font-editorial text-[34px] tracking-[-.035em] text-[#1d3933] md:text-[42px]">{documentTypeLabels[documentType] ?? request.documentType}</h1><p className="mt-3 text-[12px] text-[#69807a]">Inscrição {request.enrollment || "não informada"} · Atualizado em {new Date(request.updatedAt).toLocaleString("pt-BR")}</p></div><Badge className="w-fit rounded-full border-0 bg-[#e5f0e3] px-3 py-1.5 text-[10px] font-semibold text-[#286149]">{statusLabel[request.status] ?? request.status}</Badge></div>
    ...
    <section className="rounded-[20px] border border-[#dde4d9] bg-[#fcfbf7] p-5 md:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.15em] text-[#72887c]">Emissões e exportação</p><h2 className="mt-1 font-editorial text-[27px] text-[#213e37]">Versões documentais</h2><p className="mt-2 text-[11px] leading-5 text-[#72847b]">Cada reemissão preserva o histórico e registra uma nova versão com os dados atuais.</p></div><Button onClick={() => void issueNewVersion()} disabled={reissue.isPending || !canReissue} className="h-10 rounded-xl bg-[#234a40] text-[10px] hover:bg-[#173d36]">{reissue.isPending ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="mr-1.5 h-3.5 w-3.5" />}{latest ? "Reemitir nova versão" : "Emitir documentos"}</Button></div><div className="mt-5 overflow-x-auto"><table className="w-full min-w-[590px] text-left"><thead><tr className="border-b border-[#e3e9df] font-mono-ui text-[9px] uppercase tracking-[.13em] text-[#81938c]"><th className="pb-3 font-medium">Versão</th><th className="pb-3 font-medium">Gerada em</th><th className="pb-3 text-right font-medium">Visualizar e exportar</th></tr></thead><tbody>{versionsLoading && <tr><td colSpan={3} className="py-7 text-center text-[11px] text-[#74877f]">Carregando versões…</td></tr>}{!versionsLoading && versions.length === 0 && <tr><td colSpan={3} className="py-7 text-center text-[11px] text-[#74877f]">Ainda não há documento emitido para este processo.</td></tr>}{versions.map((version) => <tr key={version.id} className="border-b border-[#edf0e9] last:border-0"><td className="py-4 text-[11px] font-bold text-[#34564b]">Versão {version.versionNumber}</td><td className="py-4 text-[10px] text-[#74877f]">{new Date(version.createdAt).toLocaleString("pt-BR")}</td><td className="py-4"><div className="flex justify-end gap-2">{version.pdfUrl && <><Button type="button" variant="outline" size="sm" onClick={() => setPreviewUrl(version.pdfUrl!)} className="h-8 rounded-lg border-[#dbe3d8] bg-white text-[9px] text-[#426055]"><Eye className="mr-1 h-3 w-3" />Prévia</Button><Button asChild variant="outline" size="sm" className="h-8 rounded-lg border-[#dbe3d8] bg-white text-[9px] text-[#426055]"><a href={version.pdfUrl} download target="_blank" rel="noreferrer"><Download className="mr-1 h-3 w-3" />PDF</a></Button></>}{version.docxUrl && <Button asChild variant="outline" size="sm" className="h-8 rounded-lg border-[#dbe3d8] bg-white text-[9px] text-[#426055]"><a href={version.docxUrl} download target="_blank" rel="noreferrer"><FileText className="mr-1 h-3 w-3" />DOCX</a></Button>}</div></td></tr>)}</tbody></table></div></section><Dialog open={Boolean(previewUrl)} onOpenChange={(open) => { if (!open) setPreviewUrl(undefined); }}><DialogContent className="max-w-6xl gap-3 border-[#d7e2d5] bg-[#fbfbf7] p-5"><DialogHeader><DialogTitle className="font-editorial text-[26px] text-[#213e37]">Pré-visualização do documento emitido</DialogTitle><DialogDescription className="text-[11px] leading-5 text-[#708179]">Confira a versão em PDF antes de baixar, aprovar ou reemitir o documento.</DialogDescription></DialogHeader>{previewUrl && <iframe title="Pré-visualização do documento emitido em PDF" src={getPdfPreviewUrl(previewUrl)} className="h-[68vh] w-full rounded-xl border border-[#d9e2d7] bg-white" />}</DialogContent></Dialog></div>;
}
