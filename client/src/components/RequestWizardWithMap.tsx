...
type Output = { docx: { storageUrl: string }; pdf: { storageUrl: string } };
...
export default function RequestWizardWithMap() {
  ...
  const processSources = async () => {
    ...
    try {
      ...
      setStep(3);
      toast.success("Cruzamento concluído. Confira a geometria no mapa antes da emissão.");
  ...
  const produce = async () => {
    ...
    try {
      ...
      setOutput({ docx: { storageUrl: result.docx.storageUrl }, pdf: { storageUrl: result.pdf.storageUrl } });
      setStep(4);
  ...
  return <div className="mx-auto max-w-[1180px] space-y-7"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="font-mono-ui text-[10px] uppercase tracking-[.17em] text-[#627d5b]">Nova instrução</p><h1 className="mt-2 font-editorial text-[34px] tracking-[-.035em] text-[#1d3933] md:text-[42px]">Nova solicitação</h1><p className="mt-3 max-w-2xl text-[13px] leading-6 text-[#69807a]">Consolide dados, fontes territoriais e insumos antes de produzir a versão documental.</p></div><Button variant="outline" onClick={() => setLocation("/")} className="rounded-xl border-[#d8e0d6] bg-[#fcfbf7] text-xs text-[#426055]">Salvar e sair</Button></div><div className="rounded-[18px] border border-[#dce4d8] bg-[#fbfbf6] px-5 py-4"><div className="mb-3 flex justify-between font-mono-ui text-[9px] uppercase tracking-[.14em] text-[#769087]"><span>Etapa {step} de 4</span><span>{percent}% concluído</span></div><Progress value={percent} className="h-1.5 bg-[#e3e9dd] [&>div]:bg-[#7ea64f]" /><div className="mt-4 grid grid-cols-4">{["Cadastro", "Insumos", "Mapa", "Emissão"].map((label, index) => <div key={label} className={`text-[10px] font-semibold ${index + 1 <= step ? "text-[#42682d]" : "text-[#8fa097]"}`}><span className={`mb-2 grid h-5 w-5 place-items-center rounded-full border text-[9px] ${index + 1 < step ? "border-[#7da74d] bg-[#7da74d] text-white" : index + 1 === step ? "border-[#7da74d] bg-[#e9f3db] text-[#4d742e]" : "border-[#ccd8cb] bg-white"}`}>{index + 1 < step ? <Check className="h-3 w-3" /> : index + 1}</span>{label}</div>)}</div></div>
    ...
    {step === 4 && <div className="rounded-[22px] border border-[#dce4d7] bg-[#fbfbf6] p-8 text-center"><span className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-[#e7f1d8] text-[#527b32]"><CheckCircle2 className="h-7 w-7" /></span><h2 className="mt-5 font-editorial text-[34px] tracking-[-.035em] text-[#213e37]">Documentos gerados para revisão.</h2><p className="mx-auto mt-3 max-w-lg text-[12px] leading-6 text-[#6f827b]">A versão foi registrada com os dados cadastrais e a conferência cartográfica atual.</p>{output && <div className="mx-auto mt-7 flex max-w-md flex-wrap justify-center gap-3"><a href={output.docx.storageUrl} className="rounded-xl bg-[#234a40] px-5 py-3 text-[11px] font-bold text-white"><Download className="mr-2 inline h-4 w-4" />Baixar DOCX</a><a href={output.pdf.storageUrl} className="rounded-xl border border-[#d4e0d1] bg-white px-5 py-3 text-[11px] font-bold text-[#486d35]"><Download className="mr-2 inline h-4 w-4" />Baixar PDF</a></div>}</div>}
