...
export const appendAssistantUnavailableMessage = (messages: ChatMessage[], error?: { message?: string }): ChatMessage[] => [
  ...messages,
  { role: "assistant", content: assistantUnavailableMessage(error) },
];
...
export default function UrbanGlobalAssistant() {
  ...
  const sendMessage = () => {
    ...
    setMessages(next);
    setDraft("");
    void chat.mutateAsync({ messages: next.filter((message): message is ChatMessage & { role: "user" | "assistant" } => message.role !== "system").slice(-10) }).catch((error) => {
      setMessages((current) => appendAssistantUnavailableMessage(current, error));
    });
  };
